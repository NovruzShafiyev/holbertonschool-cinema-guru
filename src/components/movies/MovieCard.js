...
function MovieCard({ movie }) {
	...
	const handleClick = (type) => {
		...
		if (type === "favorite") {
			if (isFavorite) {
				...
				axios.delete(`http://localhost:8080/api/titles/favorite/${movie.imdbId}`, {
					headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
				});
			} else {
				setIsFavorite(true);
				axios.post(
					`http://localhost:8080/api/titles/favorite/${movie.imdbId}`,
					{},
					{
						headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
					}
		...
		if (type === "watchlater") {
			if (isWatchLater) {
				...
				axios.delete(`http://localhost:8080/api/titles/watchlater/${movie.imdbId}`, {
					headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
				});
			} else {
				setIsWatchLater(true);
				axios.post(
					`http://localhost:8080/api/titles/watchlater/${movie.imdbId}`,
					{},
					{
						headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
					}
	...
	useEffect(() => {
		const accessToken = localStorage.getItem("accessToken");
		axios
			.get(`http://localhost:8080/api/titles/favorite/`, {
				headers: {
		...
		axios
			.get(`http://localhost:8080/api/titles/watchlater/`, {
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${accessToken}`,
				},
